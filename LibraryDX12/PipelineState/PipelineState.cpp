...
// ------------------------------------------------------------- Generic class for pipeline state operations
...
D3D12_SHADER_BYTECODE PipelineState::loadAndCompileShader(const std::wstring &name, const std::string &target, const D3D_SHADER_MACRO *defines) {
    ...
    const auto compileFlags = ApplicationImpl::getInstance().areDebugShadersEnabled() ? D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION : UINT(0);
