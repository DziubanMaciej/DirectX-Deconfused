...
public:
    VertexOrIndexBuffer(ID3D12DevicePtr device, CommandList &commandList, const void *data, UINT size, D3D12_RESOURCE_STATES resourceStateToTransition)
        : Resource(device, D3D12_HEAP_TYPE_DEFAULT, D3D12_HEAP_FLAG_NONE, size, D3D12_RESOURCE_STATE_COPY_DEST, nullptr) {
        recordGpuUploadCommands(device, commandList, data, resourceStateToTransition);
    }
};
...
public:
    VertexBuffer(ID3D12DevicePtr device, CommandList &commandList, const void *data, UINT verticesCount, UINT vertexSize)
        : VertexOrIndexBuffer(device, commandList, data, verticesCount * vertexSize, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER) {
...
public:
    ...
    IndexBuffer(ID3D12DevicePtr device, CommandList &commandList, const void *data, UINT indicesCount)
        : VertexOrIndexBuffer(device, commandList, data, indicesCount * sizeof(UINT), D3D12_RESOURCE_STATE_INDEX_BUFFER) {
        view.BufferLocation = resource->GetGPUVirtualAddress();
